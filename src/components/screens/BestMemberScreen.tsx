...
import { Search, UserCheck, Plus, ChevronLeft, ChevronRight, CheckCircle2, UserPlus, Crown } from 'lucide-react';
...
import { TOP_5_BEST_MEMBERS } from '../../data/round2Data';
...
export const BestMemberScreen: React.FC<BestMemberScreenProps> = ({
  ...
  const handleSelectMember = (id: string, name: string) => {
    ...
    if (selectedList.includes(id) || selectedList.includes(name)) {
      ...
      onSelectCandidates([...selectedList, id]);
    }
  };
    ...
    <div className="relative flex-1 flex flex-col justify-between w-full h-full min-h-0 py-3 sm:py-4 px-3 sm:px-6 overflow-y-auto custom-scrollbar pb-32 select-none">
      ...
      <div className="text-center mb-3 shrink-0 max-w-3xl mx-auto">
        ...
        <p className="font-sans-clean text-xs sm:text-sm text-amber-200 font-bold mt-1 leading-relaxed drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">
          {isRound2
            ? `Chọn ${requiredCount} ứng viên từ Top 5 để bình chọn chính thức.`
            : `Honoring outstanding individual contributions within the team. Exactly ${requiredCount} selections required.`
          }
        </p>
      </div>
      ...
      <div className="relative flex-1 flex flex-col min-h-0 max-w-4xl mx-auto w-full py-1">
        ...
        {isRound2 ? (
          ...
          /* ===== ROUND 1: Search + Full Member List ===== */
          <>
        ...
        <SelectedTray
          ...
          items={Array.from({ length: requiredCount }, (_, idx) => {
            ...
            if (isRound2) {
              const t5 = TOP_5_BEST_MEMBERS.find(m => m.id === selectedId || m.name === selectedId);
      ...
      <div className="w-full max-w-4xl mx-auto flex justify-between items-center pt-3 border-t border-amber-300/40 shrink-0 mt-3">
        ...
        <button
          ...
          onClick={() => {
            if (selectedList.length < requiredCount) {
              toast.warning(`Please select ${requiredCount} candidates before proceeding (${selectedList.length}/${requiredCount} selected)`);
              return;
            }
            soundFx.playSelect();
            onNext();
