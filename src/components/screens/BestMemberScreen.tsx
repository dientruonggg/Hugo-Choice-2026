...
export const BestMemberScreen: React.FC<BestMemberScreenProps> = ({
  ...
  // Find currently selected member object if any
  const selectedMemberObj = ALL_MEMBERS.find(
    m => m.id === selectedCandidateId || m.name === selectedCandidateId
  );

  const handleSelectMember = (member: ClubMember) => {
    soundFx.playSelect();
    onSelectCandidate(member.name);
  };
  ...
  return (
    <div className="relative flex-1 flex flex-col justify-between w-full h-full min-h-0 py-2 sm:py-3 px-3 sm:px-6">
      ...
      <div className="text-center mb-2 sm:mb-3 shrink-0">
        ...
        <p className="font-serif-display text-xs sm:text-sm text-amber-200 font-semibold mt-1 drop-shadow-[0_2px_10px_rgba(0,0,0,0.95)]">
          Search or filter across all Hugo Club members to cast your vote
        </p>
      </div>
      ...
      <div className="relative flex-1 flex flex-col min-h-0 max-w-5xl mx-auto w-full py-1">
        ...
        {/* Currently Selected Member Banner */}
        {selectedMemberObj && (
          <div className="mb-3 p-3.5 rounded-2xl bg-amber-400/30 border-2 border-amber-300/70 backdrop-blur-md flex items-center justify-between text-white animate-fade-in shadow-xl shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xs text-amber-200 uppercase font-bold tracking-wider">Your Selected Best Member</div>
        ...
        <div className="relative flex-1 min-h-0 overflow-y-auto pr-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 custom-scrollbar">
          {filteredList.length > 0 ? (
            filteredList.map((member) => {
              const isSelected = selectedCandidateId === member.id || selectedCandidateId === member.name;
