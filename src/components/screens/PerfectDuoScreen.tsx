...
import { Check, Search, Heart, Users } from 'lucide-react';
...
const MemberPicker: React.FC<MemberPickerProps> = ({
  label, color, selectedId, search, teamFilter, onSearch, onTeamFilter, onSelect
}) => {
  ...
  return (
    <div className="flex flex-col bg-black/40 p-3 rounded-2xl backdrop-blur-md border border-white/20 min-h-0 overflow-hidden h-full">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
        {list.length === 0 ? (
          <div className="text-center text-white/50 font-serif-display text-xs py-6">No members found</div>
        ) : list.map(member => {
...
export const PerfectDuoScreen: React.FC<PerfectDuoScreenProps> = ({
  ...
  return (
    <div className="relative flex-1 flex flex-col w-full h-full min-h-0 py-2 px-3 sm:px-6 overflow-hidden">
      {/* Title */}
      <div className="text-center mb-2 shrink-0">
        ...
        <p className="font-serif-display text-[0.65rem] sm:text-sm text-pink-200 mt-0.5">
      ...
      <div className="max-w-3xl mx-auto w-full mb-2 shrink-0">
        <div className="flex items-center justify-center gap-2 sm:gap-4 p-2 sm:p-3 rounded-2xl bg-pink-950/40 border border-pink-400/50 backdrop-blur-md shadow-xl text-white">
          <div className="px-3 py-1 rounded-xl bg-white/10 border border-white/20 flex-1 text-center">
            <div className="text-[0.6rem] text-pink-300 uppercase font-bold tracking-wider">Partner #1</div>
            <div className="font-serif-display text-xs sm:text-base font-bold text-amber-200 truncate">
              {memberAObj?.name || (personA ? personA : <span className="opacity-50">Not selected</span>)}
            </div>
          </div>
          <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-full bg-pink-500 text-white flex items-center justify-center font-bold shrink-0 shadow-lg animate-pulse">
            <Heart className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-current" />
          </div>
          <div className="px-3 py-1 rounded-xl bg-white/10 border border-white/20 flex-1 text-center">
            <div className="text-[0.6rem] text-pink-300 uppercase font-bold tracking-wider">Partner #2</div>
            <div className="font-serif-display text-xs sm:text-base font-bold text-amber-200 truncate">
              {memberBObj?.name || (personB ? personB : <span className="opacity-50">Not selected</span>)}
            </div>
          </div>
        </div>
      </div>
