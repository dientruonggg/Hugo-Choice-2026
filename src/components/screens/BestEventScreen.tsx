    ...
    <div className="relative flex-1 flex flex-col justify-between w-full h-full min-h-0 py-3 sm:py-4 px-3 sm:px-6 overflow-y-auto custom-scrollbar pb-32 select-none">
      ...
      <div className="text-center mb-3 shrink-0 max-w-3xl mx-auto">
        ...
        <p className="font-sans-clean text-xs sm:text-sm text-amber-200 font-bold mt-1 leading-relaxed drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">
          {isRound2
            ? `Chọn ${requiredCount} sự kiện từ Top 5 để bình chọn chính thức.`
            : `Honoring the most impactful event or activity series of the term. Exactly ${requiredCount} selections required.`
          }
        </p>
      </div>
      ...
      <div className="relative flex-1 flex flex-col min-h-0 max-w-4xl mx-auto w-full py-1">
        ...
        {/* ===== ROUND 2: Top 5 Event Cards ===== */}
        {isRound2 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-3">
            {TOP_5_BEST_EVENTS.map((event) => {
              ...
              return (
                ...
                >
                  ...
                  {event.tag && (
                    <span className={`text-[0.6rem] px-2 py-0.5 rounded-full mb-1.5 font-bold uppercase tracking-wider ${isSelected ? 'bg-slate-950/20 text-slate-900' : 'bg-amber-300/20 text-amber-300'}`}>
                      {event.tag}
                    </span>
                  )}
                  <div className="text-xs sm:text-sm font-black leading-snug line-clamp-2 w-full">{event.name}</div>
                  {event.description && (
                    <p className={`text-[0.65rem] mt-1.5 line-clamp-2 ${isSelected ? 'text-slate-800' : 'text-slate-400'}`}>{event.description}</p>
                  )}
                  {isSelected && (
          ...
          /* ===== ROUND 1: Search + Full Event List ===== */
          <>
        ...
        <SelectedTray
          ...
          items={Array.from({ length: requiredCount }, (_, idx) => {
            ...
            if (isRound2) {
              const t5 = TOP_5_BEST_EVENTS.find(e => e.id === selectedId || e.name === selectedId);
              if (t5) return { id: t5.id, name: t5.name, subLabel: t5.tag };
            }
            const eventObj = allEventsList.find(e => e.id === selectedId || e.name === selectedId);
            return eventObj ? { id: eventObj.id, name: eventObj.name, subLabel: eventObj.icon } : selectedId ? { id: selectedId, name: selectedId } : null;
          })}
          onRemove={(item) => {
      ...
      <div className="w-full max-w-4xl mx-auto flex justify-between items-center pt-3 border-t border-amber-300/40 shrink-0 mt-3">
        ...
        <button
          ...
          onClick={() => {
            if (selectedList.length < requiredCount) {
              toast.warning(`Please select ${requiredCount} events before proceeding (${selectedList.length}/${requiredCount} selected)`);
              return;
            }
            soundFx.playSelect();
            onNext();
